...
- **Dock behavior**: `dconf-frost-defaults.ini`'s `[org/gnome/shell/extensions/dash-to-dock]` section, or Dash to Dock's own GUI settings (right-click the dock → "Dash to Dock Settings"). **Redesigned (2026-08)**: moved from a centered 48px bottom dock to a compact 32px dock on the left edge (`dock-position='LEFT'`) — **VM-confirmed**, though it took a full desktop repaint (not just the Overview) to actually show the new position; the Activities Overview kept rendering the old bottom-dock layout as a stale preview for a moment after reload.
- **FROST panel menu shortcuts**: click the FROST icon in the top bar (or edit `extension.js`'s `FrostIndicator._init()`) — now has Terminal (`guake -t`), Files (`nautilus`), Settings, Lock Screen (`loginctl lock-session`), and Suspend (`systemctl suspend`) alongside the FROST OS label, not just Settings.
