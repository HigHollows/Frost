...
- [x] **Deployment & Auto-Update pack** (`frost-deploy.sh` + `frost-update.sh`/`frost-status.sh`/`frost-uninstall.sh` + 4 systemd units): master orchestrator with pre-flight checks, checkpoints/`--resume`, per-phase (not cross-phase) rollback; opt-in-only auto-updates; a local-only (never external) daily security audit; hourly cleanup; background health monitoring; `/etc/frost/frost.conf` with `bash -n` validation; conservative opt-in-flagged uninstall with a pre-removal backup — not yet VM-validated
- [x] **FROST OS Desktop** (`frost-desktop.sh`): GNOME Shell + Mutter re-skinned in FROST Iceblue — a themed desktop, deliberately not a from-scratch custom shell (see [DESKTOP.README.md](DESKTOP.README.md) for why that scope call was made). Real GTK4/Libadwaita CSS theme, a small `frost-shell` GNOME Shell extension (Steam Mode toggle, easter egg), Dash to Dock + Blur my Shell configured (not reimplemented), a generated wallpaper, and a published mockup gallery covering all 7 requested screens. **Least-verified pack in the repo** — the JS/CSS/dconf assets are unverified against a real GNOME session (only syntax-checked), and the spec's `<200MB RAM`/`<2% CPU` targets are explicitly flagged as unrealistic for GNOME Shell rather than silently claimed
...
- [ ] **VM-validate the Boot & Aesthetic, Security, Gaming/Dev, and Deployment packs** the way Phases 1-3 were: run `frost-branding.sh`/`frost-security.sh`/`frost-gaming.sh` end-to-end in the VirtualBox VM, boot on real GRUB with the theme applied, confirm Plymouth actually renders, confirm the AUR-only tools (burpsuite/metasploit/nuclei/w3af/heroic/vscode/jetbrains-toolbox/mongodb) actually install through the delegated user, and confirm `frost --mode gaming|dev` actually flips services/governor as expected. Then run the full chain via `frost-deploy.sh` itself (including a deliberately-broken phase to confirm checkpoints/`--resume` actually work, and `frost-uninstall.sh` afterward to confirm it leaves the VM's disk in a sane state). The VM's virtual GPU is a good test of `frost-gaming.sh`'s "none/virtual" GPU-detection branch specifically.
- [ ] **Actually load `frost-shell@frost-os` in a real GNOME session.** This is a different kind of unverified than the bash scripts — `extension.js`/`stylesheet.css` were checked with Node's parser and reasoned against documented APIs, but never run inside gnome-shell. First real test: `frost-desktop.sh` in the VM, log into GNOME via GDM, `gnome-extensions enable frost-shell@frost-os`, confirm the Steam Mode toggle and easter-egg keybinding actually work, and check `journalctl` for JS errors the way you'd check any other FROST script's log.
